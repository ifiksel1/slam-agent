...
The `slam-tools` MCP server (configured in `.mcp.json`) provides script execution, profile management, and learning persistence. **Always prefer MCP tools over reading scripts into context or asking the user to run commands manually.**
...
For installation (Phase 4): use `run_install_script()`.
For diagnostics: use `run_diagnostic()`.
