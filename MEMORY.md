...
- [IRIS-LIO (COIN-LIO ⊕ EllipseLIO photometric fusion)](coinlio_ellipselio_photometric_fusion.md) — LiDAR-intensity photometric residual fused into EllipseLIO's IKFoM. v1 stable+neutral on apartment (photo_scale=1e-9). v2 GATE-0 (2026-06-13): on a degenerate bag the photometric PIPELINE destabilizes the estimate non-deterministically EVEN AT ZERO WEIGHT (timing/threading, not the math) — v2 re-scoped to fix pipeline determinism before tuning. Branch feat/coinlio-photometric-fusion, container coinlio_fusion
