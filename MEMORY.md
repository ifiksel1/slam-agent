...
- [GenZ-ICP evaluation](genz_icp_evaluation.md) — GenZ-LIO unreleased. LiDAR-ONLY GenZ-ICP, after fixing broken upstream defaults (iters 100→5, voxel 0.3→0.5): 20Hz & 100% scan capture on all 12 hangar bags, sub-metre true drift on all 6 scorable (median 0.35m, beats BIEVR 5/6, 0 divergences). ⚠️ only 3/12 bags have unimpeached GT; earlier stock-config sweep RETRACTED. Deskew A/B = NULL (≤7mm) → skip IMU graft, use ArduPilot EKF3
