#!/usr/bin/env python3
# Standalone COIN-LIO trajectory sampler (ROS1 / rospy) — mirrors ellipselio_traj_sampler.py
# EXACTLY so upstream COIN-LIO numbers are directly comparable to the EllipseLIO geo/photo and
# SuperOdom / FAST-LIO results. Subscribes COIN-LIO's /Odometry (nav_msgs/Odometry, world frame
# camera_init -> body) and tracks the same FRAME-INVARIANT metrics: total path, peak excursion
# from start, final displacement-from-start (~loop closure when you return to origin), and max
# single-scan step (a divergence/jump detector). Writes a CSV and prints a summary on exit.
#
# Usage: rosrun-style:  python3 coinlio_traj_sampler.py <csv_out> [DUR_secs] [odom_topic]
import rospy, math, signal, sys, time
from nav_msgs.msg import Odometry

CSV   = sys.argv[1] if len(sys.argv) > 1 else '/tmp/coinlio_traj.csv'
DUR   = float(sys.argv[2]) if len(sys.argv) > 2 else 1200.0
TOPIC = sys.argv[3] if len(sys.argv) > 3 else '/Odometry'
...
class M:
    ...
    def odom(self, m):
        # use the message stamp so timing matches the bag's --clock sim time, not wall time
...
def main():
    rospy.init_node('coinlio_traj_sampler', anonymous=True, disable_signals=True)
    n = M()
    stop = {'v': False}
    signal.signal(signal.SIGTERM, lambda *a: stop.update(v=True))
    signal.signal(signal.SIGINT,  lambda *a: stop.update(v=True))
    # WALL-clock timeout (NOT rospy sim time): with --clock the bag's /clock jumps to an absolute
    # Unix stamp (~1.78e9 s), so a sim-time DUR check would fire instantly. time.time() is robust.
    start = time.time()
